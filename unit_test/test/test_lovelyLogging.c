...
void test_add_and_retrieve_entry_string(void) {
    ...
    TEST_ASSERT_EQUAL_STRING("   1 - \x1B[32;1mINFO:\x1B[30;1m(file:1000)\x1B[0m my message\n\r", test.string_buffer);
}
...
void test_string_too_long() {
    ...
    int length = llog_next_entry_as_string(test.string_buffer, MAX_STRING_LEN);
    TEST_ASSERT_EQUAL(MAX_STRING_LEN, length);
    // Did not overwrite any data located next to the char array:
    TEST_ASSERT_EQUAL_HEX(POST_VALUE, test.post_string_value);
}
...
void test_add_multiple_then_read() {
    ...
    llog_next_entry_as_string(test.string_buffer, MAX_STRING_LEN);
    TEST_ASSERT_EQUAL_STRING("   1 - \x1B[32;1mINFO:\x1B[30;1m(file:1000)\x1B[0m message1\n\r", test.string_buffer);
    llog_next_entry_as_string(test.string_buffer, MAX_STRING_LEN);
    TEST_ASSERT_EQUAL_STRING("   2 - \x1B[32;1mINFO:\x1B[30;1m(file:1000)\x1B[0m message2\n\r", test.string_buffer);
    llog_next_entry_as_string(test.string_buffer, MAX_STRING_LEN);
    TEST_ASSERT_EQUAL_STRING("   3 - \x1B[32;1mINFO:\x1B[30;1m(file:1000)\x1B[0m message3\n\r", test.string_buffer);
}
